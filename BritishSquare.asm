...
# GitHub:       https://github.com/brett-fitz/BritishSquare/
#
...
#
# Revisions: 
#	PR #6: dev-cleanup  - Code Cleanup
#	PR #5: dev-docs     - Documentation + added scoring methods + bug fixes
#	PR #4: fix-print    - Bug fixes for print methods
#	PR #3: dev-full     - Intial completetion
# 	PR #2: print-board  - Print Board Methods + bug fixes
# 	PR #1: const-data   - Constant + Data declerations + initial commit 
#       
#
...
is_center_error:
	addi	$t0, $zero, 12
	bne	$s2, $t0, is_cell_available 	# Is input center cell
	addi	$v0, $zero, 0
	jal 	check_center_error 		# Checking for center error
	bne	$v0, $zero, center_error 	# if error, report it
...
is_cell_occupied:
	addi	$t0, $zero, 2
	beq	$v0, $t0, invalid_occupied_location
...
write_cell:
	...
	add	$t0, $t0, $t2 		# Moving pointer to cell location
...
check_center_error_loop:
	...
	addi	$t1, $zero, 2
	bne	$t5, $t1, center_loop_done 	# Checking if cell is not empty
	addi	$t6, $t6, 1 			# Incrementing index
	addi	$t1, $zero, 25 			# Checking if loop is done
...
#################################################################
# check_availability                                            #
# 	Description: Function that checks if there are any      #
#  	more legal moves available.                             #
...
check_availability_loop:
	addi	$t3, $zero, 25 			# Checking if loop is done
	beq	$t1, $t3, no_availability

	add	$a1, $t1, $zero			# passing index
...
#  	cell to see if a potential move is legal.               #
#                                                               #
#       Parameters: $a0 --> player                              #
#                   $a1 --> index (cell location)               #
#       Return: enum                                            #
...
check_cell_availability:
	la	$t0, Board 		# Loading Board pointer
	addi	$s4, $zero, 4
	mul	$s5, $a1, $s4           # Getting index location
	add	$t0, $t0, $s5           # Moving to index location
	lw	$t4, 0($t0)             # $t4 = board[index]
	addi	$t5, $zero, 1 		# Temp val for xor instruction
	xor	$t6, $a0, $t5           # Getting enemy player
	beq	$t4, $a0, cell_occupied 	# Check player stone in cell 
	beq	$t4, $t6, cell_occupied 	# Check enemy stone in cell 
...
check_south_availability:
	...
	addi	$s4, $zero, 20          # South index offset (5*4)
...
check_east_availability:
	...
	beq	$s5, $zero, cell_done 	# No east cell so check is done
	addi	$s4, $zero, 4 		# East cell offset (1*4)
	add	$t0, $t0, $s4           # Moving pointer to east cell
	lw	$t4, 0($t0)             # Getting east cell value
	beq	$t4, $a0, cell_done 	# Checking if player stone in cell
...
print_row_loop:
	addi	$t2, $zero, 5 
	beq	$t3, $t2, print_row_done 	# Checking if loop is done 

	lw	$t6, ($t0)			# Getting board[index]

	addi	$t2, $zero, 2			# Determining if top or bottom
	rem	$t4, $t1, $t2
...
print_bottom_cell:
	...
	la	$a0, CellBorder
	jal 	print_string
	addi	$t2, $zero, -1 		# Getting index number 
	addi	$t4, $zero, 2  
	add	$t2, $t2, $t1 
	div	$t5, $t2, $t4          
	addi	$t4, $zero, 5
	mul	$t5, $t5, $t4             
	add	$t5, $t5, $t3              
	add	$a0, $zero, $t5 	# Passing index number to print
	jal 	print_int 		# Printing index number
	addi	$t2, $zero, 10 		# Determing to print one or two spaces
	slt	$t4, $t5, $t2 
...
print_row_done:
	...
	jal 	print_string
	addi	$t2, $zero, 2 		# Determing if printed row top or bottom
	rem	$t4, $t1, $t2
...
print_top_row_done:
	addi	$t2, $zero, -20 	# Moving pointer back to start of row
	add	$t0, $t0, $t2
	addi	$t1, $t1, 1 		# Incrementing row number
	j	print_board_loop 	# Continuing loop
